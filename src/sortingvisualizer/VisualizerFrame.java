...
import static sortingvisualizer.SortingVisualizer.frame;
...
public class VisualizerFrame extends JFrame {
	...
	private final int DEFAULT_SPEED = 420;
	...
	public VisualizerFrame(){
		...
		speed = new JSlider(MIN_SPEED, MAX_SPEED, DEFAULT_SPEED);
               ...
               size = new  JTextField();
               // Sv=new JLabel("Sorting visualizer");
		...
		speed.addChangeListener(new ChangeListener() {
			...
			public void actionPerformed(ActionEvent ae)  {
				sizeVal.setText(("Size: " + size.getText() + " values"));
				validate();
				SortingVisualizer.sortDataCount = Integer.parseInt(size.getText());
			}
		}); 
 ...
 sizeModifier = (int) ((getHeight()*0.9)/(squarePanels.length));
	...
	public void preDrawArray(Integer[] squares){
		squarePanels = new JPanel[SortingVisualizer.sortDataCount];
                num=new JLabel[SortingVisualizer.sortDataCount];
                for(int i=0; i<SortingVisualizer.sortDataCount; i++){
                    String a=String.valueOf(squares[i]);
                num[i]=new JLabel(a);
                }
